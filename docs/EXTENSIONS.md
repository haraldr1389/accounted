...
An extension is an opt-in module under `src/extensions/general/<id>/` that is compiled into a deployment when its id is listed in `extensions.config.json`. Once compiled in, it is active for every company on that deployment (paid capabilities can still gate it, see [API routes](#api-routes)). An extension can:
...
Licensing: the project is AGPL-3.0 with an extension exception. An extension that interacts only through the documented Extension API (`src/lib/extensions/types.ts`, `src/lib/events/types.ts` and `src/app/api/extensions/ext/[...path]/route.ts`) may be licensed on any terms, under the conditions stated in [LICENSE](../LICENSE).

## The core boundary

These rules are enforced by CI:

- **Core never imports from `@/extensions/`.** The "Check no core imports from extensions" step in `.github/workflows/core-build.yml` greps `src/lib/`, `src/app/api/` and `src/components/` for `from '@/extensions/`. The exempt paths are `src/app/api/extensions/`, `src/components/extensions/`, `src/lib/extensions/_generated/` and `src/lib/extensions/loader.ts`. `src/lib/extensions/__tests__/type-decoupling.test.ts` applies the same rule to `src/types/index.ts`.
...
Extensions can write core data (for example, bank sync ingests transactions through `ctx.services.ingestTransactions`). Anything they write follows the same accounting rules as core code: journal writes go through `src/lib/bookkeeping/engine.ts`, and period locks are respected. See the Hard Rules in `CLAUDE.md`.

## Layout

```
src/extensions/general/<id>/
...
src/components/extensions/general/<Name>Workspace.tsx   optional workspace page
src/lib/extensions/_generated/                         written by npm run setup:extensions
```
...
`scripts/generate-extension-registry.ts` finds every `manifest.json` under `src/extensions/`. It exits with an error if two manifests share an id, or if the config lists an id that has no manifest. It then writes four files to `src/lib/extensions/_generated/`:
...
- `src/extensions/<sector>/<name>/manifest.json`, with `workspace: null`, icon `Box` and `dataPattern: "core"`
...
- **Sector:** always pass `--sector general`. The script also accepts `restaurant`, `construction`, `hotel`, `tech`, `ecommerce` and `export`, but `SectorSlug` in `src/lib/extensions/types.ts` is only `'general'`, and `src/lib/extensions/sectors.ts` defines only the `general` sector. An extension scaffolded with another sector fails type-checking and never shows up in the marketplace or under `/e/`.
- **Printed next steps:** skip steps 4 and 5, which say to edit `src/lib/extensions/loader.ts` and `src/lib/extensions/sectors.ts`. The generator handles both.
- **Route URL:** the comment in the generated `api-routes.ts` gives the wrong URL. The real one is `/api/extensions/ext/<id>/<path>`.

## Manifest

Abridged from `src/extensions/general/enable-banking/manifest.json`:
...
`Extension` in `src/lib/extensions/types.ts` has `id`, `name`, `version` and optional `sector`, plus optional surfaces. Only some surfaces are read by core today:

| Surface | Read by |
|---------|---------|
| `eventHandlers` | `extensionRegistry.register()`, which subscribes each handler to the event bus |
| `apiRoutes` | The dispatcher at `src/app/api/extensions/ext/[...path]/route.ts` |
...
A minimal event-handler extension, abridged from `src/extensions/general/example-logger/index.ts`:
...
Built by `createExtensionContext()` in `src/lib/extensions/context-factory.ts`:
...
7. The company is resolved, then the paid-capability gate runs from `EXTENSION_REQUIRED_CAPABILITY` in `src/lib/entitlements/keys.ts`.
...
**Physical route files.** Vercel cron targets (`vercel.json`) and some OAuth callbacks are concrete files under `src/app/api/extensions/<id>/`. That directory may import only its own extension. Its files compile into every build, even when the extension is disabled, so a new route must check `extensionRegistry.get('<id>')` after `ensureInitialized()` and refuse when the extension is absent. `npm run check:guards` enforces both rules through `scripts/checks/extension-route-guards.mjs`.

**Generic data routes.** `src/app/api/extensions/[sector]/[slug]/data/route.ts` (GET, POST, DELETE) and `.../settings/route.ts` (GET, PATCH) store rows under `extension_id` `<sector>/<slug>` (for example `general/tic`). That is a different row set from `ctx.settings`, which uses the bare id. The client hook `src/lib/extensions/use-extension-data.ts` calls the data route. For new work, prefer `ctx.settings` behind your own `apiRoutes`.
...
```ts
// src/lib/bank-sync/trigger-sync-contract.ts (core): the shared contract
...
// src/extensions/general/enable-banking/index.ts (extension): the provider
...
// src/app/api/v1/companies/[companyId]/bank-connections/[connectionId]/sync/route.ts (core): the caller
...
- `src/lib/extensions/payment-links.ts` scans `extensionRegistry.getAll()` for a `createInvoicePaymentLink` service (provided by `stripe`).
- `src/lib/skatteverket/declaration-status.ts` resolves a `skatteverket` service.

A related variant is to register an implementation into a core registry when the module loads. The `email` extension calls `registerEmailService()` from `src/lib/email/service.ts`, whose default is a no-op service; `_example-branding` calls `registerBrandingService()` from `src/lib/branding/service.ts`.

## UI

**Workspace page.** The component named in the manifest's `workspace` is rendered at `/e/general/<id>` by `src/app/(dashboard)/e/[sector]/[slug]/page.tsx`:
...
3. `src/components/extensions/ExtensionWorkspaceLoader.tsx` renders the component inside `ExtensionWorkspaceShell`, with props `{ userId }` (`WorkspaceComponentProps` in `src/lib/extensions/workspace-registry.tsx`).

An extension without a workspace shows a placeholder instead.

**Marketplace.** The pages are `src/app/(dashboard)/extensions/page.tsx`, `[sector]/page.tsx` and `[sector]/[extension]/page.tsx`. The sidebar lists an extension only when it has both a workspace and `definition.quickAction.href`, ordered by `quickAction.order` (`getExtensionNavItems()` in `src/lib/extensions/sectors.ts`).

**Settings panels.** Register the component by hand in `src/lib/extensions/settings-panel-registry.tsx`, as a `next/dynamic` import keyed by extension id. Core pages render it with `getSettingsPanel('<id>')`, for example `src/app/(dashboard)/import/page.tsx` and `src/components/settings/sections/BankingSettingsContent.tsx`. The panel usually loads and saves through the extension's own `apiRoutes`.

**Strings.** Manifest names and descriptions are Swedish. Translated names come from `src/lib/extensions/i18n.ts`, which maps slugs to keys in the `extensions` namespace of `src/messages/sv.json` and `src/messages/en.json`, falling back to the manifest. Put new UI strings in both message files.

## Whitelabel forks

`src/extensions/general/_example-branding/` is a starter for forks:
...
Tests live in `src/extensions/general/<id>/__tests__/`. Run them with `npx vitest run src/extensions/general/<id>`. Helpers come from `tests/helpers.ts` (`createMockSupabase()`, `createQueuedMockSupabase()`, `createMockRequest()`, fixture factories).

- **Event handlers.** Import the Extension object and take the handler from `eventHandlers`. Mock the modules it touches, typically `@/lib/supabase/server`, then call it with a payload. Example: `src/extensions/general/document-extraction/__tests__/handler.test.ts`.
- **API routes.** Find the route in `apiRoutes`, build a `Request` and a hand-made `ExtensionContext`, and call `route.handler(request, ctx)`. Example: `src/extensions/general/enable-banking/__tests__/capability-gate.test.ts`.
- **Registry and dispatcher.** These are covered by `src/lib/extensions/__tests__/registry.test.ts` and `src/app/api/extensions/ext/[...path]/__tests__/route.test.ts`.
- **Suite hygiene.** Tests that go through the bus should call `vi.clearAllMocks()` and `eventBus.clear()` in `beforeEach`.

## Common mistakes

1. **Editing `src/lib/extensions/_generated/` by hand.** The next `setup:extensions` run overwrites it.
